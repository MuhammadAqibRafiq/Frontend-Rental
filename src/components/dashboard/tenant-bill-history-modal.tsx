...
import { useTenantBills } from "@/hooks/use-bills";
...
export function TenantBillHistoryModal({ tenantId, tenantName }: TenantBillHistoryModalProps) {
  ...
  const { data: bills = [], isLoading } = useTenantBills(tenantId, open);
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-border">
                {bills.map((bill) => (
                  <tr key={bill.id} className="transition-colors hover:bg-muted/30">
                    ...
                    <td className="py-3 pr-4 text-right whitespace-nowrap font-medium text-emerald-600">
